...
// การอัปเดตข้อมูลโปรไฟล์เมื่อฟอร์มถูกส่ง
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateSql = "UPDATE users SET first_name = ?, last_name = ?, sex = ?, age = ?, province = ?, email = ? WHERE id = ?";
    $stmt = $conn->prepare($updateSql);
    $stmt->bind_param("ssssssi", $firstName, $lastName, $sex, $age, $province, $email, $userId);

    if ($stmt->execute()) {
        echo "<script>alert('ข้อมูลโปรไฟล์ถูกอัปเดตแล้ว'); window.location.href='profile.php';</script>";
    } else {
